...
import { mockSprintToEdit } from '../../../lib/data';

export default function EditSprintPage() {
  const router = useRouter();

  const [title, setTitle] = useState(mockSprintToEdit.title);
  const [description, setDescription] = useState(mockSprintToEdit.description);
  const [startDate, setStartDate] = useState(mockSprintToEdit.startDate);
  const [endDate, setEndDate] = useState(mockSprintToEdit.endDate);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log('Sprint modifié :', updatedSprint);

    // TODO : PUT vers /api/sprints/[id]
